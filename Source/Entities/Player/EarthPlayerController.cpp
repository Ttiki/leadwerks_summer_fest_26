#include "Leadwerks.h"
#include "EarthPlayerController.h"

using namespace Leadwerks;

EarthPlayerController::EarthPlayerController()
{
	//name = "EarthPlayerController";
}

void EarthPlayerController::Start()
{
...
void EarthPlayerController::Kill(shared_ptr<Entity> attacker)
{
	auto entity = GetEntity();
...
void EarthPlayerController::ToggleFlashlight()
{
...
void EarthPlayerController::ShowFlashlight()
{
...
void EarthPlayerController::HideFlashlight()
{
	if (flashlight)
	{
...
bool EarthPlayerController::ProcessEvent(const Event& e)
{
...
void EarthPlayerController::Update()
{
...
std::any EarthPlayerController::CallMethod(shared_ptr<Component> caller, const WString& name, const std::vector<std::any>& args)
{
...
void EarthPlayerController::UpdateFootsteps()
{
	auto entity = GetEntity();
	auto world = entity->GetWorld();
	if (not world) return;
...
void EarthPlayerController::UpdateFlashlight()
{
	if (flashlight)
	{
...
void EarthPlayerController::Damage(const int amount, shared_ptr<Entity> attacker)
{
...
void EarthPlayerController::Collide(shared_ptr<Entity> collidedentity, const Vec3& position, const Vec3& normal, const float speed)
{}

//This method will work with simple components
shared_ptr<Component> EarthPlayerController::Copy()
{
	auto copy = std::make_shared<EarthPlayerController>(*this);
	auto entity = GetEntity();
	if (entity)
	{
		auto world = entity->GetWorld();
...
bool EarthPlayerController::Load(table& properties, shared_ptr<Stream> binstream, shared_ptr<Map> scene, const LoadFlags flags, shared_ptr<Object> extra)
{
	auto entity = GetEntity();
	auto world = entity->GetWorld();
	...
	if (camera == NULL and world != NULL)
	{
		...
		Assert(camera, "EarthPlayerController: Failed to create camera!");
	}
...
bool EarthPlayerController::Save(table& properties, shared_ptr<Stream> binstream, shared_ptr<Map> scene, const SaveFlags flags, shared_ptr<Object> extra)
{
